...
public class EmployeeFilter {
	...
	public static void main(String[] args) {
		...
		Map<String, List<String>> collect3 = emps.stream()
				.flatMap(e -> e.getCities().stream().map(x -> Map.entry(x, e))).collect(Collectors.groupingBy(
						x -> x.getKey(), Collectors.mapping(x -> x.getValue().getFirstName(), Collectors.toList())));

		System.out.println("===========================================");
		System.out.println("printing city wise employee list : ");
		for (Entry<String, List<String>> entry : collect3.entrySet()) {
		...
		Map<String, List<CityEmployee>> collect2 = emps.stream()
				.flatMap(e -> e.getCities().stream().map(city -> new CityEmployee(city, e)))
				.collect(Collectors.groupingBy(CityEmployee::getCity));
